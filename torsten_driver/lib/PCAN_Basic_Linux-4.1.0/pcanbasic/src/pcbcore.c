...
TPCANStatus pcanbasic_errno_to_status(int err) {
	...
	case EBADMSG:
		...
		break;
	default:
		pcanlog_log(LVL_DEBUG, "Error unhandled errno (%d / %x)\n.", err, err);
	...
	if (sts == PCAN_ERROR_OK) {
		...
		if (timestamp != NULL) {
			...
			if (timestamp->millis != millis)
				timestamp->millis_overflow = (millis - timestamp->millis) >> sizeof(timestamp->millis);
			else
...
TPCANStatus pcanbasic_read_fd(
	...
	if (sts == PCAN_ERROR_OK && timestamp != NULL) {
		*timestamp = ((__u64) t.tv_sec) * 1000 + t.tv_usec;
	}
