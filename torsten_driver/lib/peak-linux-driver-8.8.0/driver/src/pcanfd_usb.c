						   ...
						   struct pcan_timeval *tv)
{
	pr_info(DEVICE_NAME ": %s(%s CAN%u, ts=%u)\n",
		__func__, dev->adapter->name, dev->nChannel+1, ts_low);
				...
				u16 op_req)
{
	...
	int i, l, err = pcan_usbfd_send_ucan_cmd(dev);
	if (err)
		return err;
				...
				u16 op_req)
{
	...
	int err = pcan_copy_from_user(&io_cfg, opt->value, sizeof(u32), c);
	if (err) {
					 ...
					 struct ucan_msg *rx_msg, void *arg)
{
	...
	DPRINTK(KERN_DEBUG
		"%s: got status msg: ts=0x%08x-%08x RB=%u EP=%u EW=%u BO=%u\n",
		DEVICE_NAME,
