			 ...
			 struct ucan_msg *rx_msg, void *arg)
{
	...
	pr_debug(DEVICE_NAME ": %s(CAN%u): bus_state=%u "
		"ts32=0x%08x bus_load=%u\n",
		dev->adapter->name, dev->nChannel+1, dev->bus_state,
...
int ucan_encode_msg(struct pcandev *dev, u8 *buffer_addr, int buffer_size)
{
	...
	for (i = 0; i < UCAN_TEST_TX_BURST; i++ ) {
		...
		/* loopback:
		 * frame copied in rx path but not written on the bus */
		if (tx.msg.flags & PCANFD_MSG_SLF)
			tx_flags |= UCAN_MSG_HW_SRR;

		/* echo:
		 * frame copied in rx path and writen on the bus */
		else if (tx.msg.flags & PCANFD_MSG_ECHO)
