				...
				const char *prompt, int offset, int len)
{
	...
	for (i = 0; i <= len; ) {
		...
		printk("%02x ", readb(ucan_pci->pci.bar0_addr + i));
		if (!(++i % 16))
			printk("\n");
	}
	if (i % 16)
...
#ifdef DEBUG_IRQ_LOST
			((struct pcifd_adapter *)dev->adapter)->lnk_set[dev->nChannel]++;
#endif
#ifdef DEBUG_IRQ_TX
...
#ifdef DEBUG_IRQ_LOST
		((struct pcifd_adapter *)dev->adapter)->lnk_irq[dev->nChannel]++;
#endif
...
static int __pcifd_driver_req_irq(struct pcandev *dev)
{
	...
	pr_info(DEVICE_NAME ": %s(CAN%u) irq_flags=%0xh\n",
		__func__, dev->nChannel+1, irq_flags);
#endif
...
static int pcifd_dev_probe(struct pcifd_adapter *ucan_pci, int c)
{
	...
	pcan_set_dev_adapter(dev, &ucan_pci->pci.adapter);
...
#ifdef PCIEFD_64BITS_CMD_MUST_BE_ATOMIC
	pcan_lock_init(&ucan_pci->ucan_lock);
#endif
